...
function openClinicCaseEdit(caseId){
  ...
  openModal(`<div class="modal-head"><div><div class="modal-kicker">${editorKicker}</div><div class="modal-title">Editează cazul · #${c._monthlyNum||c.id}</div></div><button class="modal-close" type="button">×</button></div>
...
function printFisaPDF(c){
  ...
  const doc=frame.contentWindow.document;
  doc.open();
  doc.write(`<!doctype html><html><head><title>Fișă caz #${escHTML(c._monthlyNum||c.id)}</title><style>
...
function previewFisaPDF(c){
  ...
  openModal(`<div class="modal-head"><div class="modal-title">Fișă caz #${c._monthlyNum||c.id} · ${c.name}</div><button class="modal-close" type="button">×</button></div><div class="modal-body" style="padding:0;overflow:auto;max-height:72vh"><div style="padding:16px;background:#F3F4F6">${html}</div></div><div class="modal-foot"><button class="btn modal-close" type="button">Închide</button><button class="btn" id="pdfPrintBtn" type="button">Printează PDF</button><button class="btn primary" id="pdfDlBtn" type="button">Descarcă PDF</button></div>`,'modal-wide');
