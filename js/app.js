...
function applySidebarRoles(){
  ...
  document.querySelectorAll('.nav-item[data-roles]').forEach(item=>{
    ...
    if(roles.includes(user.role))item.classList.add('role-visible');
    else item.classList.remove('role-visible');
  });
...
function renderTechnicianPortal(){
  ...
  const stages=['design','cam','prelucrare','ceramica'];const idx=stages.indexOf(myStage);
...
function buildFisaHTML(c){
  ...
  return `<div style="font-family:Arial,sans-serif;color:#1a1a1a;font-size:11px;line-height:1.5;width:750px">
<div style="background:#1a1a1a;color:white;padding:14px 24px;display:flex;justify-content:space-between;align-items:center"><div><div style="font-weight:bold;font-size:14px">LAB CAD · Laborator Dentar</div><div style="font-size:9px;opacity:0.7;margin-top:2px">Chișinău · contact@labdentar.md · +373 22 000 000</div></div><div style="font-family:monospace;font-size:18px;font-weight:bold">#${c.id}</div></div>
<div style="padding:24px 28px">
<div style="margin-bottom:14px"><div style="font-size:9px;text-transform:uppercase;letter-spacing:0.6px;color:#888;font-weight:bold;padding-bottom:4px;border-bottom:1.5px solid #1a1a1a;margin-bottom:8px">Pacient & Clinică</div><div style="display:flex;gap:24px;font-size:10px"><span>Pacient: <b>${safe(c.name)}</b></span><span>Clinică: <b>${safe(cl.name)}</b></span><span>Medic: <b>${safe(c.doctor||cl.doctor)}</b></span></div></div>
<div style="margin-bottom:14px"><div style="font-size:9px;text-transform:uppercase;letter-spacing:0.6px;color:#888;font-weight:bold;padding-bottom:4px;border-bottom:1.5px solid #1a1a1a;margin-bottom:8px">Date</div><div style="display:flex;gap:24px;font-size:10px"><span>Intrată: <b>${safe(c.intrata)}</b></span><span>Probă: <b>${safe(c.probaDate||'—')}</b></span><span>Finală: <b>${safe(c.finala)}</b></span></div></div>
<div style="margin-bottom:14px"><div style="font-size:9px;text-transform:uppercase;letter-spacing:0.6px;color:#888;font-weight:bold;padding-bottom:4px;border-bottom:1.5px solid #1a1a1a;margin-bottom:8px">Lucrare</div><div style="display:flex;gap:24px;font-size:10px"><span>Tip: <b>${safe(c.type)}</b></span><span>Culoare: <b>${safe(c.color||'—')}</b></span><span>Amprentă: <b>${safe(c.amprentaType||'—')}</b></span>${c.implantType?`<span>Implant: <b>${safe(c.implantType)}</b></span>`:''}</div></div>
<div style="margin-bottom:14px"><div style="font-size:9px;text-transform:uppercase;letter-spacing:0.6px;color:#888;font-weight:bold;padding-bottom:4px;border-bottom:1.5px solid #1a1a1a;margin-bottom:8px">Schema dentară (FDI)</div><div style="background:#f5f5f5;padding:10px;border-radius:4px"><table style="border-collapse:collapse;width:100%;table-layout:fixed">${trow(upper)}${trow(lower)}</table>${Object.keys(byType).length?`<div style="display:flex;gap:14px;flex-wrap:wrap;font-size:9px;margin-top:8px">${Object.entries(byType).map(([t,ns])=>`<span><span style="display:inline-block;width:8px;height:8px;${swatches[t]};vertical-align:middle;margin-right:4px"></span>${labels[t]} (${ns.length}): ${ns.sort((a,b)=>a-b).join(', ')}</span>`).join('')}</div>`:''}</div></div>
<div style="margin-bottom:18px"><div style="font-size:9px;text-transform:uppercase;letter-spacing:0.6px;color:#888;font-weight:bold;padding-bottom:4px;border-bottom:1.5px solid #1a1a1a;margin-bottom:8px">Indicații speciale</div><div style="font-size:10px;min-height:40px">${safe(c.notes||'Fără indicații suplimentare.')}</div></div>
<div style="display:flex;justify-content:space-between;margin-top:30px"><div style="width:220px"><div style="border-bottom:0.5px solid #1a1a1a;height:24px"></div><div style="font-size:8px;color:#888;margin-top:4px">Semnătura medic clinică</div></div><div style="width:220px"><div style="border-bottom:0.5px solid #1a1a1a;height:24px"></div><div style="font-size:8px;color:#888;margin-top:4px">Semnătura tehnician</div></div></div>
<div style="text-align:right;font-size:8px;color:#888;margin-top:14px">Emis ${new Date().toLocaleDateString('ro-RO')}</div>
</div></div>`;
}
