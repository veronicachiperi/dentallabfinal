...
async function sbRequireAuth() {
  ...
  if (!data.session) {
    if (!location.pathname.includes('login.html') && !location.pathname.includes('landing.html') && !location.pathname.includes('index.html')) location.href = 'index.html';
    return null;
  }
...
function postProcessCase(c) {
  ...
  if (!c.notStarted && typeof getEtapeLabStages === 'function') {
    ...
    if (c.stage === 'terminat' || c.stage === 'trimis') {
      ...
      // Doar etapele DINAINTE de cea curentă sunt finalizate.
